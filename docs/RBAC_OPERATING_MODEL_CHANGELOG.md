...
| Commit | Summary |
|--------|---------|
| _(pending)_ | Driver extraction Phase 4A: dispatcher driver-detail routes moved `/driver/:id` → `/fleet-driver/:id` (`/analytics`, `/profile` too) because `/driver/*` now belongs to the Pulse Driver web app. `registry/org.ts`: `org.driver-entity` / `-analytics` / `-profile` patterns updated to `/fleet-driver/…` with the **same** `fleet_management` grant, deny target and priority; `*-legacy` entries keep `/driver/:id…` (those routes only redirect). No capability, grant or gate semantics changed; `ModelAccessGate` untouched (it has no route patterns) |
