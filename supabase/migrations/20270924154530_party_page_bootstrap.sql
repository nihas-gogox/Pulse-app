-- Client + supplier (party) detail page bootstrap.
-- Index-scoped (client_id / supplier_id / contact_id) — no org-wide trip scan.
-- Non-midnight timestamp to avoid pulse-unified-base collision.
...
BEGIN
  ...
  SELECT to_jsonb(c), c.linked_organization_id
  INTO v_client, v_linked
  ...
  RETURN (
    ...
    scoped_ids AS (
      ...
      SELECT tr.id
      FROM public.trips tr
      WHERE v_linked IS NOT NULL
        AND tr.deleted_at IS NULL
        AND tr.organization_id = v_linked
...
COMMENT ON FUNCTION public.get_client_page_bootstrap(uuid, uuid) IS
  'Index-scoped client detail bootstrap: identity + that client''s trips/ledger only.';
...
BEGIN
  ...
  SELECT to_jsonb(s), s.linked_organization_id
  INTO v_supplier, v_linked
  FROM public.suppliers s
    ...
    scoped_trips AS (
      SELECT DISTINCT ON (tr.id)
        ...
        CASE
          ...
          WHEN v_linked IS NOT NULL AND tr.organization_id = v_linked THEN COALESCE(tr.supplier_rate, tr.client_price)
          ELSE NULL
...
COMMENT ON FUNCTION public.get_supplier_page_bootstrap(uuid, uuid) IS
  'Index-scoped supplier/party detail bootstrap: identity + that party''s trips/ledger only.';
