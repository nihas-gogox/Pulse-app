...
import { TRIP_REASSIGN_STALE_ERROR } from "../utils/tripReassignConflict.util";
import { shouldMarkAssignedOnFirstAssign } from "../utils/tripReassign.util";
import {
  applyIndentCommerceOrigin,
  indentIdsForCommerceLookup,
} from "../utils/applyIndentCommerceOrigin";
import {
  selectTripIndentLineageLabel,
  selectTripOperationalReference,
} from "../../operations/numbering";
import { getTripOperationalDisplayCode } from "../../operations/display";
...
import { shouldFallbackTripsTableScan } from "../utils/tripOrgFetch.util";
  ...
  if (options?.changedBy != null && updatedTrip) {
    ...
    if (auditRow) {
      const { postAssignmentUpdateAfterTripSave } = await import(
        "../../chat/services/chatAssignmentBridge.service"
      );
...
export async function stampTripDriverPayFromTerms(
  ...
  const { tripEarningsDetailForDriver } = await import(
    "../../drivers/utils/driverUtils.util"
  );
...
export async function assignTripDriverByPhone(
  ...
  const { ensureDriverRowByPhone } =
    await import("../../drivers/services/drivers.service");
